...
public class SpinningCryingShieldFeatureRenderer extends RenderLayer<CrierEntityRenderState, CrierEntityModel<CrierEntityRenderState>> {
    ...
    @Override
    public void submit(PoseStack matrices, SubmitNodeCollector queue, int light, CrierEntityRenderState state, float limbAngle, float limbDistance) {
        if (state.shieldHealth > 0) {
            ...
            state2.submit(matrices, queue, light, OverlayTexture.NO_OVERLAY, 0);
