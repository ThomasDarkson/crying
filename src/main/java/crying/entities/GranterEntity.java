        ...
        if (!world.isClientSide()) {
            ...
            if (!descending) {
                if (tickCount < FOLLOW_TICKS) {
                    ...
                    Vec3 targetEntityPos = target.position().add(0, 3d + bob, 0);

                    double speed = 0.4d;
                    Vec3 newPos = this.position().lerp(targetEntityPos, speed);
                    this.setPos(newPos);
                } 
                else {
                    tickCount = 0;
                ...
                Vec3 targetEntityPos = target.position().add(0, 1.0, 0);
                Vec3 direction = targetEntityPos.subtract(currentPos).normalize().scale(0.2d);
